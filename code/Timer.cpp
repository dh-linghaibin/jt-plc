...
PwmOut :: PwmOut(PinName pin, uint32_t frequency, TIM_TypeDef* timer, TimerChannel channel) : GPIO(pin, Pin_AF), Timer(timer)
{
	...
	if(index == 0) {
		...
		m_timer->CCMR2 |= (TIM_CCMR1_OC1M_1 | TIM_CCMR1_OC1M_2) << ((~m_channel & 0x01) * 8);
	}
...
void PwmOut :: write(uint8_t value)
{
	...
	switch(m_channel) {
		case Channel_1:
			m_timer->CCR1 = pulseWidth;
			break;
		case Channel_2:
			m_timer->CCR2 = pulseWidth;
			break;
		case Channel_3:
			m_timer->CCR3 = pulseWidth;
			break;
		case Channel_4:
			m_timer->CCR4 = pulseWidth;
			break;
	}
